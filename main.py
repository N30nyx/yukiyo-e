...
if sys.platform.startswith("win") == False:
    ui.host = "0.0.0.0"
app.config["url"] = f"http://{ui.host}:{ui.port}"
...
def render_page(content,theme,ao="",at="",ath="",af="",next="",title="",back="",t="page",name="",ep=""):
  # 'active-link'
  if theme == None:
    theme = "white"
  url = app.config["url"] + "/"
  return render_template(f"{t}.html",content=content,ao=ao,at=at,ath=ath,af=af,theme=theme,next=next,title=title,back=back,url=url,name=name,ep=ep)
...
def dlv():
    ...
    if name == None or ep == None:
        return "specify args"
    html = """\n\n<title>""" +  name + "- Episode " + ep + """</title>\n
    <style>
    .view_div {
    ...
    """
    e = "'"
    path = "downloads/" + name + "/" + ep + "/"
    path = path.replace(e,"")
    path = path.replace("'","")

    for file in os.listdir(path):
        with open( path + file,"r") as xyz:
            xyz = xyz.read()
            html += f"<div class='view_div'>\n\n<img src='{xyz}' width='700' height='1000.0' class='centeral'/>\n\n</div>\n"
    return render_page(content=html,theme=request.cookies.get("theme"),title="",next=str(int(ep)+1),back=str(int(ep)-1),t="viewer",name=name,ep=ep)
...
def downloads():
    ...
    if request.method == "POST":
        if request.form.get("command") == "dl":
            img = 1
            title = request.form.get("title")
            ep = request.form.get("ep")
            name = request.form.get("name")
            files = []
            for file in os.listdir( pathify("downloads/")):
                files.append(file)
            if name not in files:
                os.mkdir( pathify("downloads/" + name))
            files = []
            for file in os.listdir( pathify(f"downloads/{name}/")):
                files.append(file)
            if name not in files:
                os.mkdir( pathify("downloads/" + name + "/" + ep + "/"))
            s = w.series(str(title))
            imgs = w.episode(s, ep)
            for obj in imgs:
                bsf = w.baseimg(imgs[obj])
                with open( pathify("downloads/" + name + "/" + ep  + "/" + obj + ".b64"),"w+") as f:
                    f.write(bsf)
                img += 1
            return
    ...
    <h2>Downloads:</h2>
        ...
        if request.args.get("name") == None:
            t = """
        <div class='prev'><a href='""" + app.config["url"] + """/downloads/?name={name}'>{name}</div>
            """
            for file in os.listdir( pathify("downloads/")):
                r = t
                r = r.replace("{name}",file)
                html += r
        else:
            t = """
        <div class='prev'><a href='""" + app.config["url"] + """/dlv/?name={name}&ep={ep}'>{namez}</div>
            """
            name = request.args.get("name")
            e = '"'
            name = name.replace(e,"")
            ...
            if name not in files:
                os.mkdir( pathify("downloads/" + name))
            for file in os.listdir( pathify("downloads/" + name + "/")):
                r = t
                r= r.replace("{namez}","Episode " + file)
                r= r.replace("{name}",name)
                r= r.replace("{ep}",file)
                html += r
        html += "</body>\n</html>\n"
        return render_page(content=html,ath="active-link",theme=request.cookies.get("theme"))
@app.route('/query/')
def query():
...
.centeral {
  ...
  for obj in imgs:
    html += "\n<div class='view_div'>\n" + w.viewimg(imgs[obj]) + "\n</div>\n"
  return render_page(content=html,theme=request.cookies.get("theme"),title=title,next=str(int(ep)+1),back=str(int(ep)-1),t="viewer",name=name,ep=ep)
...
.episode {
  ...
  <div class="hero-text">
    ...
    <a href='""" + app.config["url"] + """/viewer?title={title}&ep=1?name={name}'>
    ...
    <li class="cards_item">
      <div class="card">
        ...
        <div class="card_content">
          ...
          <a href=" """ + app.config["url"] + """/viewer?title={title}&ep={ep}&name={name}">
          <button class="btn card_btn">Read</button>
          </a>
        </div>
      </div>
